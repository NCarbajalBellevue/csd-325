...
FIRE = '@'
EMPTY = ' '
WATER = '~'  # Added: Water used to create a permanent lake/firebreak.
...
def createNewForest():
    ...
    # Added: Create a lake roughly in the center of the forest.
    # The lake is permanent and acts as a firebreak that fire cannot cross.
    lake_width = 15
    lake_height = 8
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            ...
            if cell == TREE:
                ...
                # Added: Display the lake in blue.
                bext.fg('blue')
                print(WATER, end='')
            else:
                print(EMPTY, end='')
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                if forest[(x, y)] == EMPTY and (random.random() <= GROW_CHANCE):
                    ...
                    # Added: Water never changes once created.
                    nextForest[(x, y)] = WATER
                elif forest[(x, y)] == FIRE:
                    # Added: Fire spreads only to neighboring trees.
                    # Water cannot burn and blocks the spread of fire.
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            if forest.get((x + ix, y + iy)) == TREE:
                                nextForest[(x + ix, y + iy)] = FIRE

                    # Burned tree becomes an empty space.
